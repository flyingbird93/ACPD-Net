...
class GuidedAnchorHead(AnchorHead):
    ...
    def forward_single(self, x): #crop_feat, aes_feat, comp_feat):
        loc_pred = self.conv_loc(x[1])
        shape_pred = self.conv_shape(x[2])
        # loc_pred = self.conv_loc(x)
        # shape_pred = self.conv_shape(x)

        # loc_pred = self.conv_loc(aes_feat)
        # shape_pred = self.conv_shape(comp_feat)
        ada_feat = self.feature_adaption(x[0], shape_pred)
        # masked conv is only used during inference for speed-up
        if not self.training:
            mask = loc_pred.sigmoid()[0] >= self.loc_filter_thr
        else:
            mask = None
        cls_score = self.conv_cls(ada_feat, mask)
        bbox_pred = self.conv_reg(ada_feat, mask)
    ...
    def get_anchors(self,
        ...
        # print('multi_level_guided_anchors num: ', len(guided_anchors_list))
        # print('guided_anchors num: ', len(guided_anchors_list[0]))
        # print('guided_anchors num: ', len(guided_anchors_list[0]))
    ...
    def loss(self,
        ...
        all_anchor_list = images_to_levels(concat_anchor_list,
                                           num_level_anchors)
        # print(len(all_anchor_list))
        # print('anchor num: ', all_anchor_list[0].shape)  # 5, 1, 54400, 4
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            # print('proposals num: ', len(proposals))
